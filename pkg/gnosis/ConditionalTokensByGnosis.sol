...
contract ConditionalTokens is ERC1155 {
    ...
    // This array is initialized with a length equal to the outcome slot count. 
    // E.g. Condition with 3 outcomes [A, B, C] and two of those correct [0.5, 0.5, 0]. 
    // In Ethereum there are no decimal values, so here, 0.5 is represented by fractions like 1/2 == 0.5. 
    // That's why we need numerator and denominator values. Payout numerators are also used as a check of initialization. 
    // If the numerators array is empty (has length zero), the condition was not created/prepared. See getOutcomeSlotCount.
    mapping(bytes32 => uint[]) public payoutNumerators;
    /// Denominator is also used for checking if the condition has been resolved.
    //  If the denominator is non-zero, then the condition has been resolved.
